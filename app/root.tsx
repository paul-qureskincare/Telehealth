...
// Component to initialize embeddables on client-side only
function EmbedInitializer() {
  useEffect(() => {
    // This function initializes the embeddable form
    const initEmbeddables = () => {
      ...
      // Remove old script if already loaded
      ...
      script.onerror = () => {
        console.error('Failed to load embed bundle from:', script.src)
      }
      ...
      const initializeEmbeddables = function () {
        try {
          ...
          // Safely query for savvy elements
          ...
          if (embeddablesToLoad.length === 0) {
            console.warn('No embeddable elements found')
            return
          }
          ...
          // Function to show cache status indicator
          function showCacheIndicator(fromCache) {
            // Check if cache_status parameter is set to 1 in URL
            const urlParams = new URLSearchParams(window.location.search)
            const cacheStatusParam = urlParams.get('cache_status')
            
            // Only show indicator if cache_status=1 is present in URL
            if (cacheStatusParam !== '1') {
              console.log('🚫 Cache indicator disabled (cache_status != 1)')
              return
            }
            ...
            // Remove indicator after 6 seconds
            setTimeout(() => {
  ...
  return (
    <html lang="ru" suppressHydrationWarning>
      ...
      <body suppressHydrationWarning>
        <EmbedInitializer />
        <savvy id="flow_2571d52dhga9i00bfhde72a48gj" suppressHydrationWarning></savvy>
        {children}
        <ScrollRestoration />
        <Scripts />
      </body>
